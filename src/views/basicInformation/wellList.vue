    ...
    <el-row class="row-bg">
      ...
      <el-col :span="2">
        <el-select v-model="wellStatus" placeholder="请选择" style="width:130px;" filterable>
          <el-option v-for="item in wellstatus" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
      </el-col>
      <el-col :span="5">
    ...
    handleChange (val) {
      putWellDetail({well_id:this.well_id,dept_id:val[3]}).then(res=>{
        if(res.status===200){
          ...
          this.wellList.find(x=>x.id===this.well_id).belongs = this.getname(val[3])
